...
### 6. 프리미엄 PDF 리포트 시스템 (Architecture Decision)
- **방식**: **HTML Template (Jinja2) -> PDF Convert (WeasyPrint)** 방식 채택.
- **이유**:
  - `ReportLab`처럼 코드로 좌표를 찍는 방식은 디자인 수정이 매우 어렵고 생산성이 낮음.
  - HTML/CSS로 디자인을 미리 구현하고 데이터만 주입하는 방식이 훨씬 예쁘고 유지보수하기 쉬움.
  - `WeasyPrint`는 CSS 표준 지원이 우수하며 파이썬 환경 친화적임.
- **최적화**:
  - **비동기 처리**: PDF 생성 시간이 길기 때문에 `BackgroundTasks` 활용.
  - **비용 절감**: 모든 뉴스를 분석하지 않고 RAG로 핵심 뉴스만 추려서 GPT에게 전달하여 토큰 비용 절감.
